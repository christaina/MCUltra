...
class RawInput(object):
    def __init__(self, data_bundle, vocabulary=None):#, c_len=None, q_len=None):
        (self.contexts, self.questions, self.choices, self.labels,
            self.choices_map, self.context_lens, self.qs_lens, self.entities) = data_bundle
        if vocabulary:
...
class Model(object):
    ...
    def __init__(self, vocab_size, choices_idx, pre_embedding=None):
        ...
        for i in range(batch_size):
            self.ent_indices.append(
                tf.placeholder(tf.int32, shape=[None], name="entities{0}".format(i)))
            self.n_choices.append(
                tf.placeholder(tf.int32, shape=None, name="n_choices{0}".format(i)))
       ...
       # print("state shape: %s" % question_lstm.state_fw.h.get_shape())
        ...
        for i in range(batch_size):
            ...
            context_ent = tf.gather(c_outputs[i], self.ent_indices[i])
            ques = tf.expand_dims(q_state[i], dim=1)
            # norm_c = tf.nn.l2_normalize(context_entities, dim=1)
            # norm_q = tf.expand_dims(tf.nn.l2_normalize(q_state[i], 0), dim=1)
            logits = tf.matmul(context_ent, ques)
            print(logits.get_shape())
            predictions.append(tf.argmax(logits, 0))
            #correct.append(
                #tf.cast(tf.equal(predictions, self.enc_y[i][: self.n_choices[i]])), dtype=tf.float32) 
            losses.append(
...
def run_epoch(session, model, input, train_op=None, verbose=False,
    ...
    for j, batch in enumerate(input):
        ...
        questions, context, choices, labels, choices_map, context_lens, qs_lens, ent_indices = batch

...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    self._input = input_
    choices_size = len(input_.all_choices)
    ...
    lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=False)
    lstm_bw_cell = tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=False)
    ...
    self._initial_state_fw = lstm_fw_cell.zero_state(batch_size, data_type())
    self._initial_state_bw = lstm_bw_cell.zero_state(batch_size, data_type())
    ...
    output = tf.reshape(concat_outputs, [-1, size])
    print("Shape after concatting + reshaping: %s"%output.get_shape())
    softmax_w = tf.get_variable(
        "softmax_w", [size, choices_size], dtype=data_type())
    ...
    print("original y shape: %s"%input_.targets.get_shape())
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.name_scope("Train"):
      ...
      train_wdw = GenBatch(train_data_wdw)
      with tf.variable_scope("Model", reuse=None, initializer=initializer):
        m = PTBModel(is_training=True, config=config, input_=train_wdw)

...
flags.DEFINE_integer("max_epoch", 60, "Max number of epochs.")
...
flags.DEFINE_bool("use_choice_embedding", False, "Use gru cells")
...
class BiLSTM(object):
    ...
    def __init__(self, input_x, sequence_lengths, embedding,size, keep_prob,name):
        ...
        (outputs_fw, outputs_bw), (self.state_fw, self.state_bw) = \
...
class Model(object):
    ...
    def __init__(self, vocab_size, choices_idx,pre_embedding=None):
        ...
        # placeholder indices
        self.place_inds = tf.placeholder(tf.int32, [batch_size])
        ...
        choices_embedding = tf.nn.embedding_lookup(embedding_drop, self.choices_idx)
        print(choices_embedding.get_shape())

        # bidirectional lstm - context
        context_lstm = BiLSTM(self.c_x, self.c_lengths, embedding_drop,\
                size,self.keep_prob,name='context')
        c_outputs = context_lstm._outputs

        # bidirectional lstm - question
        question_lstm = BiLSTM(self.q_x, self.q_lengths, embedding_drop,\
                size,self.keep_prob,name="question")
        q_state = question_lstm._state
        print('question state shape: %s'%q_state.get_shape())
        q_outputs = question_lstm._outputs
        place_inds = tf.unpack(self.place_inds,axis=0)
        print(place_inds[0])
        print(2*size)
        q_outputs = tf.unpack(q_outputs,axis=0)
        place_inds_sliced = [tf.slice(x,[place_inds[i],0],[1,int(2*size)]) \
                for i,x in enumerate(q_outputs)]
        q_state = tf.squeeze(tf.pack(place_inds_sliced,axis=0))
        print('place shape: %s'%q_state.get_shape())
        print(len(q_outputs))
...
def run_epoch(session, model, input, train_op=None, verbose=False,
    ...
    for j, batch in enumerate(input):
        ...
        questions, context, choices, labels, choices_map, context_lens,\
                qs_lens,place_ind = batch
        ...
        feed_dict[model.place_inds] = place_ind
        if is_testing:
        ...
        with tf.Session() as session:
            ...
            for i in range(FLAGS.max_epoch):
                train_iter = rn.batch_iter(
                    ...
                    train.qs_lens, batch_size=FLAGS.batch_size,place_inds=train.placeholder_inds)
                ...
                val_iter = rn.batch_iter(
                    ...
                    val.qs_lens, batch_size=FLAGS.batch_size,place_inds=val.placeholder_inds)
                ...
                test_iter = rn.batch_iter(
                    ...
                    test.qs_lens, batch_size=FLAGS.batch_size,place_inds=test.placeholder_inds)
